 ...
 * 关于单个表格的actiongroup
 */
public class VBActionGroup extends AbstractActionGroup
{
	private ViewModel selection;
	private ConnControl conn;
	public VBActionGroup(TreeViewer tv)
	{
		super(tv);
		selection = (ViewModel) ((IStructuredSelection)treeViewer.getSelection()).getFirstElement();
	...
	class DelTable extends Action
	{
		...
		public void run()
		{
			if(MessageDialog.openConfirm(activeShell, "确认删除", "请确定是否删除该视图？"))
			{
				String sql = "use " + selection.getParent().getParent().getName() + "\r\n";
				conn.execute(sql);	// 先选择数据库，执行查询
				if(!conn.execute("drop view " + selection.getName()))
				{
					MessageBox mb = new MessageBox(activeShell);
					mb.setText("提示");
					mb.setMessage("删除视图有误!");
					mb.open();
					return;
				}
				else
	...
	{
		// 可能已经重命名了
		conn.refreshVB(selection.getParent());
		treeViewer.refresh();
	}
}
